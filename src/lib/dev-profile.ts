...
function parseBands(value: string | undefined, fallback: DevBand[]) {
  ...
  const bands = value.split(",").reduce<DevBand[]>((currentBands, entry) => {
...
export const devProfile: DevProfile = {
  ...
  instruments: parseList(
    process.env.EXPO_PUBLIC_DEV_PROFILE_INSTRUMENTS,
    defaultProfile.instruments,
  ),
